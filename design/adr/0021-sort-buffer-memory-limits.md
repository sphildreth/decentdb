...
1.  **Complexity**: Implementing a robust external merge sort is a significant engineering task (temp file management, serialization, I/O scheduling), risky for the initial 0.x baseline.
...
2.  **In-Memory Sort Only (0.x baseline)**: We will **not** implement disk spilling in the initial 0.x baseline.
...
*   **Mitigation**: Users are encouraged to `ORDER BY` indexed columns (which don't require sorting) or increase the memory config. This is a "Decent" trade-off for an embedded engine in the initial 0.x baseline.
