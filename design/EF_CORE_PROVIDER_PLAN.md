...
- Transactions work (ambient and explicit).
- NodaTime support is available as an optional extension package.
- `AsSplitQuery` works for typical `Include(...)` patterns.
- Provider passes a meaningful subset of EF Core relational conformance tests.
...
- Migrations handle unsupported DDL operations with a documented strategy (e.g., table rebuild where required).
...
- Basic LINQ:
  ...
  - simple `Join`/`GroupJoin` (only if low-risk)
...
- Chosen approach: publish `DecentDB.AdoNet` as a standalone NuGet package (alongside `DecentDB.MicroOrm`) and have `DecentDB.EntityFrameworkCore` depend on `DecentDB.AdoNet`.
  ...
  - Requirements:
    - Adopt ADR-0093 (`design/adr/0093-dotnet-nuget-packaging-adonet-package.md`) and update/supersede ADR-0044 (`design/adr/0044-dotnet-nuget-packaging.md`), which currently documents the single-package strategy.
...
- Recommended sequencing to close issue #19 quickly: implement NodaTime support in MicroOrm first (requested scope), then mirror the same mappings/converters in `DecentDB.EntityFrameworkCore.NodaTime`. If the work produces reusable mapping/converter code, consider a shared internal helper assembly, but only with an ADR (dependency/architecture decision).
...
- Migrations design-time services first (unblocks most EF workflows).
- Scaffolding next (valuable, but a larger surface and more schema edge cases).

---

## 11. Expand Query Translation Toward “Expected Provider” Coverage
