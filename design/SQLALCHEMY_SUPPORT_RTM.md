...
| P1-06 | Provide a Python-perf extension: row batch API OR row view API | ✅ | Row view API present: `decentdb_row_view` in `src/c_api.nim` (Python binding implemented but currently disabled due to bug) |
...
| P2-05 | Implement `Cursor.executemany(sql, seq_of_params)` | ⚠️ | Implemented as loop calling `execute()`; no batching, no stmt reuse optimization yet |
...
| P2-12 | Enforce/communicate single-writer model | ⚠️ | Documented in `bindings/python/README.md`. No runtime enforcement yet. |
| P2-13 | Avoid suggesting cross-process semantics beyond DecentDB | ✅ | Documented limitations in README. |
...
| P3-03 | connect args mapping for path + query options | ✅ | `connect` kwargs support `stmt_cache_size`. Query options passed to `decentdb_open`. |
| P3-04 | SQL compilation: LIMIT/OFFSET matches DecentDB | ✅ | `DecentDbCompiler.limit_clause()` + integration test |
| P3-05 | SQL compilation: RETURNING only enabled if supported / otherwise emulated | ✅ | Explicitly disabled: `implicit_returning=False` + compiler raises `CompileError`; tested |
| P3-06 | Type compiler maps SQLAlchemy types to DecentDB storage types | ✅ | Implemented `_DecentDate`, `_DecentDateTime`, etc. Tested in `bindings/python/tests/test_types_sqlalchemy.py` |
| P3-07 | Introspection best-effort: `get_table_names`, `get_columns`, indexes, FKs | ⚠️ | `get_table_names/has_table/get_columns` implemented. Indexes/FKs partially stubbed or missing. |
...
| P4-02 | Relationship support (one-to-many/many-to-one) | ✅ | Tested in `bindings/python/tests/test_relationships.py` |
| P4-03 | Eager loading patterns (`joinedload`, `selectinload`) | ⚠️ | `joinedload` works. `selectinload` disabled due to parser limitation with `IN ($1)`. |
...
| T-06 | datetime/date/time stored as INT64 with UTC semantics | ✅ | Dialect implements `bind_processor`/`result_processor` for `Date`/`DateTime`/`Time`. Tested. |
| T-07 | decimal/Numeric stored as TEXT preserving precision | ✅ | Dialect implements `Numeric` as TEXT. Tested. |
| T-08 | uuid.UUID stored as BLOB(16) | ✅ | Dialect implements `Uuid` as BLOB. Tested. |
| T-09 | Enum stored as INT64 or TEXT (configurable) | ✅ | Dialect implements `Enum` as TEXT. Tested. |
...
| ID | Requirement | Status | Evidence |
|---|---|---|---|
| E-01 | Map native errors into DB-API exception hierarchy | ⚠️ | `_raise_error()` maps known error codes; SQL parse errors covered |
| E-02 | SQLAlchemy exceptions include SQL + params + native error code | ✅ | `_raise_error(..., sql=..., params=...)` appends JSON Context; tested |
...
| PERF-01 | Prepared statement cache (LRU) | ✅ | Implemented in `Connection` with configurable size. Tested. |
| PERF-02 | Efficient `fetchmany()` with batch-native API | ⚠️ | C API row_view implemented but Python usage currently disabled due to bug. |
...
| TEST-02 | Unit tests: type round-trips | ✅ | `bindings/python/tests/test_types_sqlalchemy.py` |
...
| TEST-05 | Benchmarks for <1ms overhead budget | ✅ | `bench_fetch.py` created and runnable. |
