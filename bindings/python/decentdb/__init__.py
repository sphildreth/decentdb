...
paramstyle = (
    "qmark"  # We accept qmark (?) or named (:name) and rewrite to $N for engine
)
...
def TimeFromTicks(ticks):
    return datetime.time(0, 0, 0)  # Simplification
...
class Cursor:
    def __init__(self, connection):
        ...
        self._has_step_with_params_row_view = hasattr(
            self._lib, "decentdb_step_with_params_row_view"
        )
    ...
    def execute(self, operation, parameters=None):
        ...
        if self._stmt:
            if self._last_sql == sql:
                ...
                if not can_defer_select:
                    ...
                    can_prove_full_rebind = ("?" in operation) or (
                        re.search(r":([a-zA-Z_][a-zA-Z0-9_]*)", operation) is not None
                    )
                    if not (
        ...
        if (
            (self._col_count or 0) > 0
            and self._use_row_view
            and self._has_step_with_params_row_view
        ):
        ...
        for i, param in enumerate(params):
            ...
            if param is None:
                ...
                b = memoryview(param).tobytes() if isinstance(param, memoryview) else param
                ArrayType = ctypes.c_uint8 * len(b)
                b_arr = ArrayType.from_buffer_copy(b)
                res = self._lib.decentdb_bind_blob(self._stmt, idx, b_arr, len(b))
        ...
        if self._pending_select:
            ...
            for i, param in enumerate(pending_params):
                ...
                if param is None:
                    ...
                    b = memoryview(param).tobytes() if isinstance(param, memoryview) else bytes(param)
                    buf = ctypes.create_string_buffer(b)
                    keepalive.append(buf)
                    v.kind = 5
                    v.isNull = 0
            ...
            if res != ERR_OK:
                _raise_error(
                    ...
                    sql=self._pending_select_sql or self._last_sql,
            ...
            for i in range(ncols):
                ...
                if k == 1:
                    ...
                    if bool(v.bytes) and v.bytesLen > 0:
                        row[i] = string_at(v.bytes, v.bytesLen).decode(
    ...
    def _get_row_view_impl(self):
        ...
        res = self._lib.decentdb_row_view(
            self._stmt, ctypes.byref(values_ptr), ctypes.byref(count)
        )
    ...
    def _get_row_slow(self):
        ...
        for i in range(col_count):
            ...
            if kind == 0:  # vkNull
                ...
                length = ctypes.c_int()
                ptr = self._lib.decentdb_column_text(
                    self._stmt, i, ctypes.byref(length)
                )
                if ptr:
                ...
                length = ctypes.c_int()
                ptr = self._lib.decentdb_column_blob(
                    self._stmt, i, ctypes.byref(length)
                )
                if ptr:
...
class Connection:
    def __init__(self, path, options="", stmt_cache_size=128):
        self._lib = load_library()
        self._db = self._lib.decentdb_open(
            path.encode("utf-8"), options.encode("utf-8")
        )
        if not self._db:
...
def evict_shared_wal(path):
    """Remove the shared WAL registry entry for a database path.

    Call this before replacing a database file on disk so that the next
    ``connect()`` to the same path creates a fresh WAL instead of reusing
    a stale one from a previous file.
    """
    lib = load_library()
    fs_path = os.fspath(path)
    if isinstance(fs_path, str):
        fs_path = fs_path.encode("utf-8")
    lib.decentdb_evict_shared_wal(fs_path)
