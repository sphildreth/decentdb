...
def test_blob_bytearray_and_memoryview(db_path):
    conn = decentdb.connect(db_path)
    cur = conn.cursor()
    cur.execute("CREATE TABLE t_blob_advanced (id INT64, data BLOB)")

    ba = bytearray(b'\x01\x02\x03')
    mv = memoryview(b'\x04\x05\x06')
    
    cur.execute("INSERT INTO t_blob_advanced VALUES (?, ?)", (1, ba))
    cur.execute("INSERT INTO t_blob_advanced VALUES (?, ?)", (2, mv))

    cur.execute("SELECT data FROM t_blob_advanced ORDER BY id")
    rows = cur.fetchall()
    
    # DecentDB might return them as bytes
    assert rows[0][0] == b'\x01\x02\x03'
    assert rows[1][0] == b'\x04\x05\x06'
