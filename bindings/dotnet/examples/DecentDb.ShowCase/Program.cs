...
class Program
{
    ...
    private static async Task DemonstrateDatabaseOperations()
    {
        ...
        Console.WriteLine("  EF setup: UseDecentDB(DecentDBConnectionStringBuilder) with NodaTime enabled");
        Console.WriteLine();
    }
    ...
    private static async Task DemonstrateBulkOperations()
    {
        ...
        var updated = await context.Tags
            ...
            .ExecuteUpdateAsync(setters => setters.SetProperty(t => t.UsageCount, t => t.UsageCount + 1));
        Console.WriteLine($"  Bulk update (ExecuteUpdateAsync): {updated} tags updated");
    ...
    private static async Task DemonstratePerformancePatterns()
    {
        ...
        Console.WriteLine("  These are sanity checks for common EF Core usage patterns, not benchmark claims.");

        Console.WriteLine("  1. Projection vs Include (avoid over-fetching):");
        List<string> projectedNames = [];
        var projectedElapsed = await MeasureAsync(async () =>
        {
        ...
        Console.WriteLine($"     Projection timing: {projectedElapsed.TotalMilliseconds:F2} ms");

        List<Product> withInclude = [];
        var trackedElapsed = await MeasureAsync(async () =>
        {
        ...
        Console.WriteLine($"     Tracked query timing: {trackedElapsed.TotalMilliseconds:F2} ms");

        Console.WriteLine("  2. AsNoTracking for read-only:");
        List<Product> noTracking = [];
        var noTrackingElapsed = await MeasureAsync(async () =>
        {
        ...
        Console.WriteLine($"     AsNoTracking timing: {noTrackingElapsed.TotalMilliseconds:F2} ms");

        Console.WriteLine("  3. Split queries (reduces cartesian products):");
        List<Order> splitQuery = [];
        var splitElapsed = await MeasureAsync(async () =>
        {
        ...
        Console.WriteLine($"     Split query: {splitQuery.Count} orders / {splitQuery.SelectMany(o => o.OrderItems).Count()} items");
        Console.WriteLine($"     Split query timing: {splitElapsed.TotalMilliseconds:F2} ms");

        Console.WriteLine("  4. Keyset pagination (efficient than offset):");
        var lastPrice = 0m;
        List<string> keysetPage = [];
        var keysetElapsed = await MeasureAsync(async () =>
        {
        ...
        Console.WriteLine($"     Keyset timing: {keysetElapsed.TotalMilliseconds:F2} ms");

        Console.WriteLine("  5. Batch size optimization:");
        Console.WriteLine("     Use AddRange with reasonable batch sizes (100-1000)");
        Console.WriteLine("     Prefer set-based updates/deletes and statement reuse where practical");

        Console.WriteLine("  6. Async streaming over query results:");
        var streamedNames = new List<string>();
        var asyncStreamElapsed = await MeasureAsync(async () =>
        {
        ...
        Console.WriteLine($"     AsAsyncEnumerable: {string.Join(", ", streamedNames)}");
        Console.WriteLine($"     Async streaming timing: {asyncStreamElapsed.TotalMilliseconds:F2} ms");
        Console.WriteLine();
    }
