...
public class DataReaderTests : IDisposable
{
    ...
    [Fact]
    public void TimestampColumns_ReportMetadataAndRoundTripCorrectly()
    {
        using var conn = new DecentDBConnection($"Data Source={_dbPath}");
        conn.Open();

        using var cmd = conn.CreateCommand();
        cmd.CommandText = "CREATE TABLE timestamp_test (occurred_at TIMESTAMP)";
        cmd.ExecuteNonQuery();

        var expected = new DateTime(2020, 3, 4, 0, 0, 0, DateTimeKind.Utc);
        var expectedMicros = (expected.Ticks - DateTime.UnixEpoch.Ticks) / 10L;

        cmd.CommandText = "INSERT INTO timestamp_test (occurred_at) VALUES (@occurredAt)";
        cmd.Parameters.Clear();
        var parameter = cmd.CreateParameter();
        parameter.ParameterName = "@occurredAt";
        parameter.Value = expected;
        cmd.Parameters.Add(parameter);
        cmd.ExecuteNonQuery();

        cmd.CommandText = "SELECT occurred_at FROM timestamp_test";
        cmd.Parameters.Clear();
        using var reader = cmd.ExecuteReader();

        Assert.True(reader.Read());
        Assert.Equal("TIMESTAMP", reader.GetDataTypeName(0));
        Assert.Equal(typeof(DateTime), reader.GetFieldType(0));
        Assert.Equal(expectedMicros, reader.GetInt64(0));
        Assert.Equal(expectedMicros, reader.GetFieldValue<long>(0));
        Assert.Equal(expected, reader.GetDateTime(0));
        Assert.Equal(expected, reader.GetFieldValue<DateTime>(0));
    ...
    [Fact]
    public void GetString_FromUuidColumn_ReturnsCanonicalGuid()
    {
        using var conn = new DecentDBConnection($"Data Source={_dbPath}");
        conn.Open();

        using var cmd = conn.CreateCommand();
        cmd.CommandText = "CREATE TABLE uuid_string_test (id INTEGER PRIMARY KEY, value UUID)";
        cmd.ExecuteNonQuery();

        var expected = Guid.NewGuid();
        cmd.CommandText = "INSERT INTO uuid_string_test (id, value) VALUES (1, @value)";
        var parameter = cmd.CreateParameter();
        ...
        cmd.CommandText = "SELECT value FROM uuid_string_test WHERE id = 1";
        cmd.Parameters.Clear();
        using var reader = cmd.ExecuteReader();

        Assert.True(reader.Read());
        Assert.Equal(expected.ToString(), reader.GetString(0));
        Assert.Equal(expected.ToString(), reader.GetFieldValue<string>(0));
    }
