...
public class AdoNetLayerTests : IDisposable
{
    ...
    [Fact]
    public void DecentDBCommand_Prepare_ReusesSingleStatementForRepeatedNonQueryExecution()
    {
        using var conn = new DecentDBConnection($"Data Source={_dbPath}");
        conn.Open();
        ...
        {
            createCmd.CommandText = "CREATE TABLE prepared_reuse (id INTEGER PRIMARY KEY, name TEXT)";
            createCmd.ExecuteNonQuery();
        }

        using var cmd = conn.CreateCommand();
        cmd.CommandText = "INSERT INTO prepared_reuse (id, name) VALUES (@p0, @p1)";

        var idParam = cmd.CreateParameter();
        idParam.ParameterName = "@p0";
        ...
        var preparedField = typeof(DecentDBCommand).GetField("_preparedStatement", BindingFlags.Instance | BindingFlags.NonPublic);
        Assert.NotNull(preparedField);

        cmd.Prepare();
        var preparedStatement = preparedField!.GetValue(cmd);
        Assert.NotNull(preparedStatement);

        idParam.Value = 1;
        nameParam.Value = "alpha";
        Assert.Equal(1, cmd.ExecuteNonQuery());
        Assert.Same(preparedStatement, preparedField.GetValue(cmd));

        idParam.Value = 2;
        nameParam.Value = "beta";
        Assert.Equal(1, cmd.ExecuteNonQuery());
        Assert.Same(preparedStatement, preparedField.GetValue(cmd));

        cmd.CommandText = "SELECT COUNT(*) FROM prepared_reuse";
