...
public sealed class QueryTranslationTests : IDisposable
{
    ...
    [Fact]
    public void Any_EmitsExistsWithLimitOne()
    {
        SeedData();
        ...
        var (result, sql) = CaptureExecutedSql(connection, () => context.Items.Any());
    ...
    [Fact]
    public void AnyWithPredicate_EmitsExistsWithLimitOne()
    {
        SeedData();
        ...
        var (result, sql) = CaptureExecutedSql(connection, () => context.Items.Any(x => x.Id >= 5));
    ...
    private static (T Result, string Sql) CaptureExecutedSql<T>(DecentDBConnection connection, Func<T> operation)
    {
        string? sql = null;

        void HandleSqlExecuted(object? _, SqlExecutedEventArgs args)
        {
            sql = args.Sql;
        }
        ...
        try
        {
            ...
            return (result, Assert.IsType<string>(sql));
        }
        finally
        {
            connection.SqlExecuted -= HandleSqlExecuted;
