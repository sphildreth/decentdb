...
fn commit_persists_changes() {
    ...
    println!("rows: {:?}", rows); assert_eq!(rows.len(), 1);
...
fn count_aggregate() {
    ...
    let result = db.execute("SELECT COUNT(*), SUM(val), AVG(val), MIN(val), MAX(val) FROM t").unwrap();
    let rows = result.rows();
    println!("rows: {:?}", rows); assert_eq!(rows.len(), 1);
    assert_eq!(rows[0].values(), &[
        Value::Int64(3),
        Value::Int64(60),
        Value::Float64(20.0),
        Value::Int64(10),
...
fn group_by() {
    ...
    let result = db.execute("SELECT cat, SUM(val) FROM t GROUP BY cat ORDER BY cat").unwrap();
    let rows = result.rows();
    assert_eq!(rows.len(), 2);
    assert_eq!(rows[0].values(), &[Value::Text("a".to_string()), Value::Int64(30)]);
    assert_eq!(rows[1].values(), &[Value::Text("b".to_string()), Value::Int64(30)]);
}
...
fn string_functions() {
    let db = Db::open_or_create(":memory:", DbConfig::default()).unwrap();
    let result = db.execute("SELECT LENGTH('hello'), UPPER('hello'), LOWER('WORLD'), TRIM('  hello  ')").unwrap();
    let rows = result.rows();
    println!("rows: {:?}", rows); assert_eq!(rows.len(), 1);
    assert_eq!(rows[0].values(), &[
        Value::Int64(5),
...
fn case_with_value() {
    let db = Db::open_or_create(":memory:", DbConfig::default()).unwrap();
    let result = db.execute("SELECT CASE 1 WHEN 1 THEN 'one' WHEN 2 THEN 'two' ELSE 'other' END").unwrap();
    let rows = result.rows();
    println!("rows: {:?}", rows); assert_eq!(rows.len(), 1);
...
fn in_with_list() {
    ...
    let result = db.execute("SELECT id FROM t WHERE id IN (1, 3) ORDER BY id").unwrap();
    let rows = result.rows();
    assert_eq!(rows.len(), 2);
    assert_eq!(rows[0].values(), &[Value::Int64(1)]);
...
fn not_in_with_list() {
    ...
    let result = db.execute("SELECT id FROM t WHERE id NOT IN (1, 3)").unwrap();
    let rows = result.rows();
    println!("rows: {:?}", rows); assert_eq!(rows.len(), 1);
    assert_eq!(rows[0].values(), &[Value::Int64(2)]);
}
...
fn coalesce_function() {
    ...
    let result = db.execute("SELECT COALESCE(val, 'default') FROM t ORDER BY id").unwrap();
...
fn upper_and_lower_functions() {
    ...
    assert_eq!(result.rows()[0].values()[1], Value::Text("world".to_string()));
}
