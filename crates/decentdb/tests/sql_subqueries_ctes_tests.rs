...
fn complex_cte_recursive_with_data() {
    let db = mem_db();
    db.execute("CREATE TABLE categories(id INT64, parent_id INT64, name TEXT)")
        .unwrap();
...
fn complex_cte_with_join_and_aggregate() {
    let db = mem_db();
    db.execute("CREATE TABLE departments(id INT64 PRIMARY KEY, name TEXT)")
        .unwrap();
    db.execute(
        "CREATE TABLE employees(id INT64 PRIMARY KEY, name TEXT, dept_id INT64, salary INT64)",
    )
    .unwrap();
...
fn correlated_scalar_subquery() {
    ...
    db.execute("CREATE TABLE t1 (id INT64 PRIMARY KEY, val INT64)")
        .unwrap();
    db.execute("CREATE TABLE t2 (id INT64, score INT64)")
        .unwrap();
    db.execute("INSERT INTO t1 VALUES (1, 100), (2, 200)")
        .unwrap();
    db.execute("INSERT INTO t2 VALUES (1, 10), (1, 20), (2, 30)")
        .unwrap();
...
fn correlated_subquery_in_select() {
    ...
    db.execute("INSERT INTO t1 VALUES (1,'A'),(2,'B'),(3,'A')")
        .unwrap();
    db.execute("INSERT INTO t2 VALUES ('A','alpha'),('B','beta')")
        .unwrap();
    let r = db
        .execute(
...
fn correlated_subquery_in_where() {
    let db = mem_db();
    db.execute("CREATE TABLE orders(id INT64, customer_id INT64, amount INT64)")
        .unwrap();
...
fn correlated_subquery_with_between() {
    let db = mem_db();
    db.execute("CREATE TABLE ranges (id INT64 PRIMARY KEY, lo INT64, hi INT64)")
        .unwrap();
    db.execute("CREATE TABLE vals (v INT64)").unwrap();
    db.execute("INSERT INTO ranges VALUES (1, 10, 20), (2, 30, 40)")
        .unwrap();
    db.execute("INSERT INTO vals VALUES (15), (35), (50)")
        .unwrap();
...
fn correlated_subquery_with_in_list() {
    let db = mem_db();
    db.execute("CREATE TABLE parent (id INT64 PRIMARY KEY, code TEXT)")
        .unwrap();
    db.execute("CREATE TABLE child (pid INT64, tag TEXT)")
        .unwrap();
    db.execute("INSERT INTO parent VALUES (1, 'a'), (2, 'b')")
        .unwrap();
    db.execute("INSERT INTO child VALUES (1, 'a'), (1, 'x'), (2, 'y')")
        .unwrap();
...
fn correlated_subquery_with_like() {
    let db = mem_db();
    db.execute("CREATE TABLE patterns (id INT64 PRIMARY KEY, pat TEXT)")
        .unwrap();
    db.execute("CREATE TABLE strings (val TEXT)").unwrap();
    db.execute("INSERT INTO patterns VALUES (1, 'a%'), (2, 'b%')")
        .unwrap();
    db.execute("INSERT INTO strings VALUES ('apple'), ('banana'), ('cherry')")
        .unwrap();
...
fn delete_with_in_subquery() {
    ...
    exec(
        &db,
        "DELETE FROM dws_main WHERE val IN (SELECT val FROM dws_del)",
    );
...
fn delete_with_subquery() {
    ...
    db.execute("DELETE FROM t1 WHERE id IN (SELECT ref_id FROM t2)")
        .unwrap();
...
fn delete_with_subquery_in_where() {
    ...
    db.execute("INSERT INTO t1 VALUES (1),(2),(3),(4),(5)")
        .unwrap();
    db.execute("INSERT INTO t2 VALUES (2),(4)").unwrap();
    db.execute("DELETE FROM t1 WHERE id IN (SELECT id FROM t2)")
        .unwrap();
...
fn exists_subquery() {
    ...
    let r = exec(
        &db,
        "SELECT id FROM esq WHERE EXISTS (SELECT 1 FROM esq WHERE val = 10)",
    );
    assert_eq!(r.rows().len(), 2);
}
...
fn exists_with_empty_subquery() {
    ...
    let r = db.execute("SELECT EXISTS (SELECT 1 FROM t)").unwrap();
...
fn generated_column_rejects_subquery() {
    let db = mem_db();
    let err = db
        .execute("CREATE TABLE t (id INT64, gen INT64 GENERATED ALWAYS AS ((SELECT 1)) STORED)")
        .unwrap_err();
    assert!(
        err.to_string().to_lowercase().contains("generated")
            || err.to_string().to_lowercase().contains("subquer")
    );
}
...
fn group_by_with_in_subquery_having() {
    ...
    exec(
        &db,
        "INSERT INTO gh_sub VALUES ('a', 1), ('b', 2), ('c', 3)",
    );
...
fn group_by_with_scalar_subquery_in_select() {
    ...
    exec(
        &db,
        "INSERT INTO gh_ss VALUES ('a', 10), ('a', 20), ('b', 30)",
    );
...
fn in_subquery_normalization() {
    ...
    let result = db
        .execute("SELECT * FROM t WHERE id IN (SELECT id FROM t WHERE id > 1) ORDER BY id")
        .unwrap();
    let rows = rows(&result);
    assert_eq!(rows.len(), 2);
}
...
fn in_subquery_with_multiple_values() {
    let db = mem_db();
    db.execute("CREATE TABLE t(id INT64, category TEXT)")
        .unwrap();
    db.execute("CREATE TABLE categories(name TEXT)").unwrap();
    db.execute("INSERT INTO t VALUES (1,'A'),(2,'B'),(3,'C'),(4,'A')")
        .unwrap();
    db.execute("INSERT INTO categories VALUES ('A'),('C')")
        .unwrap();
    let r = db
...
fn parse_exists_subquery() {
    ...
    db.execute("CREATE TABLE t2(id INT64, t1_id INT64)")
        .unwrap();
    db.execute("INSERT INTO t1 VALUES (1), (2), (3)").unwrap();
    db.execute("INSERT INTO t2 VALUES (10, 1), (20, 3)")
        .unwrap();
    let r = db
        .execute(
...
fn recursive_cte_max_iteration_limit() {
    ...
    assert!(
        err.contains("1000") || err.contains("limit") || err.contains("iteration"),
...
fn recursive_cte_rejects_case_with_subquery() {
    let db = mem_db();
    db.execute("CREATE TABLE aux (id INT64 PRIMARY KEY)")
        .unwrap();
    let err = db
        .execute(
            "WITH RECURSIVE cte AS (
...
fn scalar_subquery_in_group_by_select() {
    ...
    exec(
        &db,
        "INSERT INTO ssgs VALUES (1, 'eng'), (2, 'eng'), (3, 'sales')",
    );
    exec(
        &db,
        "INSERT INTO ssgs_counts VALUES ('eng', 2), ('sales', 1)",
    );
    let r = exec(
        &db,
...
fn scalar_subquery_in_select() {
    ...
    let r = exec(
        &db,
        "SELECT id, (SELECT MAX(val) FROM ssq) AS mx FROM ssq ORDER BY id",
    );
...
fn scalar_subquery_returning_null() {
    let db = mem_db();
    db.execute("CREATE TABLE t(id INT64)").unwrap();
    let r = db.execute("SELECT (SELECT id FROM t LIMIT 1)").unwrap();
...
fn subquery_exists() {
    let db = mem_db();
    db.execute("CREATE TABLE orders(id INT64, customer_id INT64)")
        .unwrap();
    db.execute("CREATE TABLE customers(id INT64, name TEXT)")
        .unwrap();
    db.execute("INSERT INTO customers VALUES (1,'Alice'),(2,'Bob'),(3,'Charlie')")
        .unwrap();
    db.execute("INSERT INTO orders VALUES (100, 1),(101, 1),(102, 2)")
        .unwrap();
    let r = db
        .execute(
...
fn subquery_in_from_with_aggregation() {
    let db = mem_db();
    db.execute("CREATE TABLE t(grp TEXT, val INT64)").unwrap();
    db.execute("INSERT INTO t VALUES ('A',1),('A',2),('B',3),('B',4),('B',5)")
        .unwrap();
    let r = db
        .execute(
...
fn update_with_subquery_in_set() {
    ...
    db.execute("CREATE TABLE t1(id INT64 PRIMARY KEY, val INT64)")
        .unwrap();
    db.execute("CREATE TABLE t2(id INT64, bonus INT64)")
        .unwrap();
    db.execute("INSERT INTO t1 VALUES (1, 100)").unwrap();
    db.execute("INSERT INTO t2 VALUES (1, 50)").unwrap();
    db.execute("UPDATE t1 SET val = val + (SELECT bonus FROM t2 WHERE t2.id = t1.id) WHERE id = 1")
        .unwrap();
...
fn update_with_subquery_in_where() {
    ...
    db.execute("INSERT INTO t1 VALUES (1,10),(2,20),(3,30)")
        .unwrap();
    db.execute("INSERT INTO t2 VALUES (1),(3)").unwrap();
    db.execute("UPDATE t1 SET val = val * 10 WHERE id IN (SELECT id FROM t2)")
        .unwrap();
...
fn where_in_subquery_with_and() {
    let db = mem_db();
    db.execute("CREATE TABLE t1(id INT64, active BOOLEAN)")
        .unwrap();
    db.execute("CREATE TABLE t2(t1_id INT64)").unwrap();
    db.execute("INSERT INTO t1 VALUES (1,TRUE),(2,FALSE),(3,TRUE)")
        .unwrap();
    db.execute("INSERT INTO t2 VALUES (1),(3)").unwrap();
    let r = db
        .execute(
