...
mod tests {
    ...
    #[test]
    fn stale_index_fixture_detected() {
        use crate::metadata::SchemaTableInfo;
        let mut data = fresh_collected();
        if let Some(ref mut schema) = data.schema {
            schema.tables.push(SchemaTableInfo {
                name: "t".into(),
                temporary: false,
                ddl: "CREATE TABLE t(id INT64, v TEXT)".into(),
                row_count: 0,
                primary_key_columns: vec![],
                checks: vec![],
                foreign_keys: vec![],
    ...
    #[test]
    fn fragmentation_fixture_detected() {
        let mut data = fresh_collected();
        data.database.page_count = 200;
        if let Some(ref mut hdr) = data.header {
            hdr.freelist_page_count = 50;
        }
        let findings = evaluate_rules(&data);
        assert!(findings.iter().any(|f| f.id == "fragmentation.high"));
    }
    ...
    #[test]
    fn golden_markdown_empty_report_stable() {
        let r = report(vec![]);
        let md = render_markdown(&r);
        let lines: Vec<&str> = md.lines().collect();
        assert_eq!(lines[0], "# DecentDB Doctor Report");
        assert_eq!(lines[1], "");
    ...
    #[test]
    fn golden_json_empty_report_schema_version_1() {
        let r = report(vec![]);
        ...
        assert_eq!(v["summary"]["error_count"], 0);
        assert_eq!(v["fixes"].as_array().unwrap().len(), 0);
    }

    #[test]
    fn golden_json_schema_version_is_always_1() {
        let r = report(vec![finding(
            "wal.large_file",
            DoctorSeverity::Warning,
            DoctorCategory::Wal,
        ...
        assert!(json.contains("\"id\": \"wal.large_file\""));
    }

    #[test]
    fn fix_execution_uses_only_catalog_actions() {
        let all_ids: std::collections::HashSet<&str> = [
            "fix.checkpoint",
            "fix.rebuild_stale_index",
        ...
        .copied()
        .collect();
        // Every finding we produce should plan a fix only from the catalog.
        let findings = vec![
            finding(
                "wal.large_file",
                DoctorSeverity::Warning,
            ...
            index_not_fresh("stale", "t"),
        ];
        let fixes = plan_fixes(&findings);
        for fix in &fixes {
            assert!(
                all_ids.contains(fix.id.as_str()),
                "unexpected fix id: {}",
                fix.id
            );
        }
    ...
    #[test]
    fn collected_facts_populated_for_full_report() {
        use crate::Db;
        use crate::DbConfig;
        use tempfile::TempDir;
        let dir = TempDir::new().expect("temp dir");
        let path = dir.path().join("facts_test.ddb");
        Db::create(&path, DbConfig::default()).expect("create");
        let report = run_doctor(&path, DoctorOptions::default()).expect("doctor");
        let cf = &report.collected;
        assert!(cf.storage.is_object());
        assert!(cf.header.is_object());
        assert!(cf.schema.is_object());
    }
}
