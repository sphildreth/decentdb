...
def normalize_radar(engines):
    ...
    for key, label, direction in METRICS:
        ...
        if direction == "lower":
            ...
            best_val = values.min()
            normalized[label] = values.apply(
                lambda x: best_val / x if pd.notna(x) and x > 0 else np.nan
            )
        else:
            ...
            best_val = values.max()
            normalized[label] = values.apply(
                lambda x: x / best_val if pd.notna(x) and best_val > 0 else np.nan
            )
...
def plot_radar(normalized_df):
    ...
    plt.yticks(
        [0.25, 0.5, 0.75, 1.0], ["0.25", "0.50", "0.75", "1.00"], color="grey", size=7
    )
    ...
    for i, engine in enumerate(normalized_df.index):
        values = normalized_df.loc[engine].fillna(0).values.flatten().tolist()
...
def normalize_speedup(engines):
    ...
    for key, label, direction in METRICS:
        ...
        if direction == "lower":
            ...
            normalized[label] = values.apply(
                lambda x: base_val / x if pd.notna(x) and x > 0 else np.nan
            )
        else:
            # Throughput: Speedup = Value / Baseline
            # e.g. Baseline 100ops, Target 200ops -> 200/100 = 2.0x speedup
            normalized[label] = values.apply(
                lambda x: x / base_val if pd.notna(x) and base_val > 0 else np.nan
            )
...
def plot_speedup(speedup_df):
    ...
    all_engines = list(speedup_df.index)
    engines = []
    for e in ["DecentDB", BASELINE_ENGINE]:
        if e in all_engines:
            engines.append(e)
    ...
    for i, engine in enumerate(engines):
        ...
        y_pos = y + (len(engines) - 1 - i) * height

        # Map string metrics to indices
        # ax.barh handles NaN properly by skipping them
        ax.barh(y_pos, vals, height, label=engine)

        # Add value labels
        for j, v in enumerate(vals):
            if pd.notna(v):
                ax.text(v + 0.05, y_pos[j], f"{v:.2f}x", va="center", size=9)
    ...
    ax.axvline(
        ...
        linestyle="--",
        linewidth=1,
        label=f"{BASELINE_ENGINE} Baseline (1.0x)",
    )
