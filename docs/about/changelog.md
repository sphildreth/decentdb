...
- **Phase D row-source seam (ADR 0145):** `EngineRuntime.tables` now stores a `TableRowSource` abstraction instead of exposing `Arc<TableData>` directly, and the executor now has a first `TableRowIter` / visible-row-source layer for several simple scan, join, and runtime-index read paths. This slice is still resident-only, but it lands the compatibility seam needed to migrate scan-heavy executor paths onto paged table iteration without another flag-day refactor.
