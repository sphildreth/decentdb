...
### Fixed
- **Engine**: Resolved a thread synchronization race condition during shared WAL initialization. Waiter threads now properly wait (`os.sleep`) for the primary initializer instead of causing assertion failures.
- **Engine / Memory**: Fixed 5 memory leak findings across the engine, query planner, schema cache, and transaction commit paths:
  - Fixed orphaned Pager caches on transaction aborts.
  - Fixed memory leak in unhandled recursive CTE execution flows.
...
### Added
- **Engine**: Shared WAL registry for cross-connection visibility. Multiple connections to the same on-disk database now share a single WAL instance, allowing reads on one connection to immediately see commits from another without reconnecting. See ADR-0117.
- **Engine**: Shared `maxPageCount` atomic counter in the WAL. Pages allocated by one connection are recognized as valid by all other connections sharing the WAL, preventing "Cannot free page id" errors in concurrent multi-connection workloads.
- **C-API**: New `decentdb_evict_shared_wal(path)` function to forcibly remove a shared WAL registry entry before replacing a database file on disk (e.g. backup import, recovery).
- **Python Bindings**: New `decentdb.evict_shared_wal(path)` module-level function exposing the C-API WAL eviction.
...
### Fixed
- **C-API**: Added a call to `malloc_trim(0)` inside `decentdb_close` on Linux to force glibc to release cached memory back to the OS immediately, resolving resident set size (RSS) bloat in long-running processes that repeatedly open and close databases.
- **Tests**: Added C-API and Python leak regression tests to strictly measure and bound memory consumption limits on process termination loops.
