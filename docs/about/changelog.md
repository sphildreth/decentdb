...
- **Per-table on-demand loading activation (ADR 0143 Phase B follow-up):** wired the per-table loader into the SQL read paths via a new strict whitelist analyzer `safe_referenced_tables` (`crates/decentdb/src/sql/ast.rs`). The analyzer returns `Some(tables)` only when the statement's table reference set is provably exhaustive (plain `SELECT`/`UPDATE`/`DELETE`/`INSERT … VALUES` with subquery-free expressions and `Table`/`Join` FROM items); CTEs, set operations, `INSERT … SELECT`, FROM-subqueries/functions, view/CTE references, and any expression-level subquery (`InSubquery`/`CompareSubquery`/`ScalarSubquery`/`Exists`) conservatively fall back to `ensure_all_tables_loaded`. `prepare()` no longer eagerly materializes all tables — row data is loaded on first execution. `EngineRuntime::rebuild_stale_indexes` now skips indexes whose target table is still deferred so a per-table load no longer causes "table data for X is missing" corruption errors. **Per-table load remains opt-in** via `DbConfig::defer_table_materialization` (default `false`); flipping the default will be safe once the WAL snapshot is pinned across the `persisted_tables` read and the overflow-payload read (see the test `concurrent_writer_reader_overflow_consistency` for the failure mode in concurrent workloads).
