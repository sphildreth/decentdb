...
- **Phase D row-source seam + first paged deferred scan slice (ADR 0145):** `EngineRuntime.tables` now stores a `TableRowSource` abstraction instead of exposing `Arc<TableData>` directly, the executor has a `TableRowIter` / visible-row-source layer for several simple scan, join, and runtime-index read paths, and non-transaction deferred reads can now answer plain `COUNT(*)`, grouped `COUNT(*)`, plain `SELECT *` / `alias.*` sequential scans, the simplest filtered-projection queries, and the narrow grouped numeric aggregate fast path from `TableRowSource::Paged` without promoting the table to resident `Vec<StoredRow>`. This still rides on manifests over the legacy table payload bytes; broader scan/aggregate migration and append-only paged persistence remain follow-up work.
- **Phase D first paged-persistence slice (ADR 0145):** format 10 now introduces a table-level paged manifest behind `DbConfig::paged_row_storage` (default `false`). Existing legacy tables can be backfilled into that manifest on open, dirty tables written under the flag now persist behind bounded **multi-chunk** paged manifests, and `decentdb-migrate` can copy v9 databases forward to the new format. This is still the first persistence slice only: writes rebuild paged chunks from the resident table image today, with true append-only row pages, tombstones, and compaction-aware persistence still follow-up work.
