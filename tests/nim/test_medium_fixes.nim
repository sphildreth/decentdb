...
suite "MED-001: B-Tree Internal Node Binary Search":
  ...
  test "Binary search finds correct child in internal nodes":
    ...
    for i in 1..numRows:
      let insertRes = execSql(db, "INSERT INTO test (id, data) VALUES ($1, $2)", 
        @[Value(kind: vkInt64, int64Val: int64(i)), Value(kind: vkText, bytes: toBytes("xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx"))])
      if not insertRes.ok:
        echo "Error at i=", i, ": ", insertRes.err
      check insertRes.ok
    ...
    for i in 1..numRows:
      let selectRes = execSql(db, "SELECT id FROM test WHERE id = $1", 
  ...
  test "Binary search with non-sequential inserts":
    ...
    for key in keys:
      let insertRes = execSql(db, "INSERT INTO test2 (id, data) VALUES ($1, $2)",
...
suite "MED-003: Trigram Delta Flush During Checkpoint":
  ...
  test "Multiple transactions batched before checkpoint":
    ...
    for i in 1..5:
      ...
      let insertRes = execSql(db, "INSERT INTO docs (id, body) VALUES ($1, $2)",
...
suite "MED-004: Clock Eviction Mark-and-Compact":
  ...
  test "Cache eviction with small cache triggers tombstone path":
    ...
    for i in 1..20:
      let selectRes = execSql(db, "SELECT data FROM test WHERE id = $1",
        @[Value(kind: vkInt64, int64Val: i)])
      check selectRes.ok
      check selectRes.value.len == 1
  ...
  test "Clock compaction triggered at tombstone threshold":
    ...
    for i in 1..10:
      let insertRes = execSql(db, "INSERT INTO test3 (id, blob) VALUES ($1, $2)",
    ...
    for iteration in 1..3:
      for i in 1..10:
        let selectRes = execSql(db, "SELECT id FROM test3 WHERE id = $1",
          @[Value(kind: vkInt64, int64Val: i)])
        check selectRes.ok
...
suite "MED-006: WAL Index Binary Search":
  ...
  test "Binary search with concurrent reader snapshot":
    ...
    for i in 1..5:
      ...
      let updateRes = execSql(db, "UPDATE test2 SET counter = $1 WHERE id = 1",
        @[Value(kind: vkInt64, int64Val: i)])
      check updateRes.ok
...
suite "MED Combined: All optimizations work together":
  ...
  test "All optimizations work in integration scenario":
    ...
    for i in 1..50:
      let insertRes = execSql(db, "INSERT INTO articles (id, title, body) VALUES ($1, $2, $3)",
        @[Value(kind: vkInt64, int64Val: i), 
    ...
    let btreeQueryRes = execSql(db, "SELECT id, title, body FROM articles WHERE title = 'Title 25'")
  ...
  test "Performance: Verify optimizations have expected characteristics":
    ...
    for i in 1..100:
      let insertRes = execSql(db, "INSERT INTO perf_test (id, data) VALUES ($1, $2)",
        @[Value(kind: vkInt64, int64Val: int64(i)), Value(kind: vkText, bytes: toBytes("xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx"))])
      check insertRes.ok
