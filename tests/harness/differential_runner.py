...
DIFFERENTIAL_TESTS = [
    ...
    DifferentialTest(
        ...
        schema_sql="CREATE TABLE test (id INT PRIMARY KEY, dept TEXT, score INT)",
        setup_sql="INSERT INTO test VALUES (1, 'eng', 95), (2, 'eng', 80), (3, 'sales', 90), (4, 'sales', 85)",
        test_sql="SELECT id, dept, score, ROW_NUMBER() OVER (PARTITION BY dept ORDER BY score DESC) AS rn FROM test ORDER BY dept, score DESC",
        expect_rows=["2|eng|80|2", "1|eng|95|1", "4|sales|85|2", "3|sales|90|1"],
        ignore_order=False,
    ),
    DifferentialTest(
        name="window_rank",
        description="RANK() with ties",
        schema_sql="CREATE TABLE test (id INT PRIMARY KEY, score INT)",
        setup_sql="INSERT INTO test VALUES (1, 95), (2, 95), (3, 90), (4, 85), (5, 80)",
        test_sql="SELECT id, score, RANK() OVER (ORDER BY score DESC) AS rnk FROM test ORDER BY score DESC, id",
        expect_rows=["1|95|1", "2|95|1", "3|90|3", "4|85|4", "5|80|5"],
        ignore_order=False,
    ),
    DifferentialTest(
        name="window_dense_rank",
        description="DENSE_RANK() with ties (no gaps)",
        schema_sql="CREATE TABLE test (id INT PRIMARY KEY, score INT)",
        setup_sql="INSERT INTO test VALUES (1, 95), (2, 95), (3, 90), (4, 85), (5, 80)",
        test_sql="SELECT id, score, DENSE_RANK() OVER (ORDER BY score DESC) AS drnk FROM test ORDER BY score DESC, id",
        expect_rows=["1|95|1", "2|95|1", "3|90|2", "4|85|3", "5|80|4"],
        ignore_order=False,
    ),
    DifferentialTest(
        name="window_lag",
        description="LAG() window function with default",
        schema_sql="CREATE TABLE test (id INT PRIMARY KEY, score INT)",
        setup_sql="INSERT INTO test VALUES (1, 95), (2, 90), (3, 85), (4, 80)",
        test_sql="SELECT id, score, LAG(score, 1, 0) OVER (ORDER BY score DESC) AS prev_score FROM test ORDER BY score DESC",
        expect_rows=["1|95|0", "2|90|95", "3|85|90", "4|80|85"],
        ignore_order=False,
    ),
    DifferentialTest(
        name="window_lead",
        ...
        schema_sql="CREATE TABLE test (id INT PRIMARY KEY, score INT)",
        setup_sql="INSERT INTO test VALUES (1, 95), (2, 90), (3, 85), (4, 80)",
        test_sql="SELECT id, score, LEAD(score) OVER (ORDER BY score DESC) AS next_score FROM test ORDER BY score DESC",
        expect_rows=["1|95|90", "2|90|85", "3|85|80", "4|80|"],
        ignore_order=False,
    ),
    DifferentialTest(
        name="window_rank_partition",
        description="RANK() with PARTITION BY",
        schema_sql="CREATE TABLE test (id INT PRIMARY KEY, dept TEXT, score INT)",
        setup_sql="INSERT INTO test VALUES (1, 'eng', 95), (2, 'eng', 95), (3, 'eng', 80), (4, 'sales', 90), (5, 'sales', 85)",
        test_sql="SELECT id, dept, score, RANK() OVER (PARTITION BY dept ORDER BY score DESC) AS rnk FROM test ORDER BY dept, score DESC, id",
        expect_rows=["1|eng|95|1", "2|eng|95|1", "3|eng|80|3", "4|sales|90|1", "5|sales|85|2"],
        ignore_order=False,
    ),
    DifferentialTest(
        name="window_multiple_functions",
        description="Multiple window functions in one query",
        schema_sql="CREATE TABLE test (id INT PRIMARY KEY, score INT)",
        setup_sql="INSERT INTO test VALUES (1, 95), (2, 90), (3, 85)",
        test_sql="SELECT id, score, ROW_NUMBER() OVER (ORDER BY score DESC) AS rn, RANK() OVER (ORDER BY score DESC) AS rnk, DENSE_RANK() OVER (ORDER BY score DESC) AS drnk FROM test ORDER BY score DESC",
        expect_rows=["1|95|1|1|1", "2|90|2|2|2", "3|85|3|3|3"],
        ignore_order=False,
    ),
]
